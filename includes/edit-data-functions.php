...
function simple_reygiri_edit_page()
{
    global $errorMessage;
    global $wpdb;
        ...
        <form method="post" action="" id="simple-reygiri-settings-form">
            <table class="form-table" id="simple-reygiri-table">
                ...
                <tr valign="top">
                    ...
                    <td>
                        <input type="submit" name="add_row" value="افزودن" class="button-secondary" />
                    </td>
                </tr>
                <?php
                foreach ($existing_data as $data) {
                    echo '<tr valign="top">' .
                        // '<td class="firstCol">' . esc_attr($data['id']) . '</td>' .
                        '<td><input type="text" name="setting1[' . esc_attr($data['id']) . ']" value="' . esc_attr($data['setting1']) . '" /></td>' .
                        '<td><input type="text" name="setting2[' . esc_attr($data['id']) . ']" value="' . esc_attr($data['setting2']) . '" /></td>' .
                        '<td><input type="text" name="setting3[' . esc_attr($data['id']) . ']" value="' . esc_attr($data['setting3']) . '" /></td>' .
                        '<td><input type="text" name="setting4[' . esc_attr($data['id']) . ']" value="' . esc_attr($data['setting4']) . '" /></td>' .
                        '<td><input type="text" name="setting5[' . esc_attr($data['id']) . ']" value="' . esc_attr($data['setting5']) . '" /></td>' .
                        '<td><input type="text" name="setting6[' . esc_attr($data['id']) . ']" value="' . esc_attr($data['setting6']) . '" /></td>' .
                        '<td>' .
                        '<input type="hidden" name="row_id" value="' . esc_attr($data['id']) . '">' .
                        '<input type="submit" name="remove_row[' . esc_attr($data['id']) . ']" value="حذف" class="button-secondary remove-row" />' .
                        '</td>' .
                        '</tr>';
                }
                ?>
                ...
                <!-- <input type="submit" name="add_row" value="Add Row" class="button-secondary" /> -->
